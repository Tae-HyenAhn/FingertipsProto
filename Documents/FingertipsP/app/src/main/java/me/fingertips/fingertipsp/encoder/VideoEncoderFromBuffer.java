...
public class VideoEncoderFromBuffer {
    ...
    private static final int BIT_RATE = 8000000; // bit rate CameraWrapper.
    private int mWidth;
    private int mHeight;
    ...
    public VideoEncoderFromBuffer(int width, int height) {
        ...
        try {
            mMuxer = new MediaMuxer(Environment.getExternalStorageDirectory().getAbsolutePath()+
                    "/FINGERTIPS/ft_"+System.currentTimeMillis()+".mp4", MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
    ...
    public void encodeFrame(byte[] input, long pTime /* , byte[] output */) {
        ...
        if (inputBufferIndex >= 0) {
            ...
            //mFrameData.length, timeStamp, 0);

            mMediaCodec.queueInputBuffer(inputBufferIndex, 0, mFrameData.length, pTime, 0);
            //mMediaCodec.queueInputBuffer(inputBufferIndex, 0, input.length, pTime, 0);

            timeStamp = timeStamp + (1000000/30);
...
//					}
                    ...
                    //timeStamp = timeStamp + (30/1000000000);
                    mMuxer.writeSampleData(mTrackIndex, outputBuffer, mBufferInfo);
                    if (VERBOSE) {
                        Log.d(TAG, "sent " + mBufferInfo.size + " bytes to muxer");
                        Log.d(TAG, "timestamp " + mBufferInfo.presentationTimeUs + " /stampe");
                    }
